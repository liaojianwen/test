...
- (instancetype)initLabelViewWithLab:(NSString *)text clickTextRange:(NSRange)clickTextRange clickTextColor:(UIColor *)clickTextColor clickAtion:(clickBlock)clickAtion{
    self = [super init];
    if (self) {
        //变量
        NSString *clickText;
        ...
        clickLabel.textColor = clickTextColor;
...
//        self.clickLab = clickLabel;
        ...
        commonLabel.backgroundColor = [UIColor redColor];
        commonLabel2.backgroundColor = [UIColor blueColor];
        _text = text;
...
//- (instancetype)clickTextRange:(NSRange)clickTextRange clickTextColor:(UIColor *)clickTextColor clickAtion:(clickBlock)clickAtion{
//    
//    NSString *clickText;
//    NSString *commonText;
//    NSString *commonText2;
...
//    
//    //如果选择的范围文字是文尾的文字，而不是中间的文字或者开头的文字
//    if (clickTextRange.location + clickTextRange.length == _text.length) {
//        clickText = [_text substringWithRange:clickTextRange];
//        commonText = [_text substringToIndex:clickTextRange.location];
//        //label的位置
//        commonLabel.frame = CGRectMake(0, 0, [[self class] calculateRowWidth:commonText], 30);
//        clickLabel.frame = CGRectMake(commonLabel.frame.size.width+commonLabel.frame.origin.x, 0, [[self class] calculateRowWidth:clickText], 30);
//    }else if (clickTextRange.location == 0) {//选择的范围文字是开头的
//        clickText = [_text substringWithRange:clickTextRange];
//        commonText = [_text substringWithRange:NSMakeRange(clickText.length, _text.length-clickText.length)];
//        
//        //label的位置
//        clickLabel.frame = CGRectMake(0, 0, [[self class] calculateRowWidth:clickText], 30);
//        commonLabel.frame = CGRectMake(clickLabel.frame.size.width+clickLabel.frame.origin.x, 0, [[self class] calculateRowWidth:commonText], 30);
...
//        clickText = [_text substringWithRange:clickTextRange];
//        commonText = [_text substringToIndex:clickTextRange.location];
//        commonText2 = [_text substringWithRange:NSMakeRange(clickTextRange.length+clickTextRange.location, _text.length-clickText.length-commonText.length)];
//        commonLabel2 = [UILabel new];
//        //label的位置
//        commonLabel.frame = CGRectMake(0, 0, [[self class] calculateRowWidth:commonText], 30);
//        clickLabel.frame = CGRectMake(commonLabel.frame.size.width+commonLabel.frame.origin.x, 0, [[self class] calculateRowWidth:clickText], 30);
...
//    
//    // self.backgroundColor = [UIColor yellowColor];
//    self.frame = CGRectMake(([UIScreen mainScreen].bounds.size.width-(clickLabel.frame.size.width+commonLabel.frame.size.width+commonLabel2.frame.size.width))/2, originY, [[self class] calculateRowWidth:_text], 30);
//    self.clickBlock = clickAtion;
//    //        self.clickLab = clickLabel;
//    [self addSubview:commonLabel];
//    [self addSubview:clickLabel];
